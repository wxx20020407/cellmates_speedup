...
class EM:
    ...
    def fit(self, X: np.ndarray, max_iter: int = 200, rtol: float = 1e-4, num_processors: int = 1,
        ...
        if num_processors > 1:
            ...
            # keep stable multiprocessing path for reproducibility
...
def fit_quadruplet(v: int, w: int, obs_vw: np.ndarray,
    ...
    logger.info(
        f"[diag][pair-start] pair={v}-{w} obs_shape={obs_vw.shape} "
        f"theta_init={theta_init_.tolist()} hmm_alg={quad_model.hmm_alg} e_step={quad_model.E_step_alg}"
    )
    ...
    while not convergence and it < max_iter:
        ...
        logger.info(
            f"[diag][iter] pair={v}-{w} iter={it + 1}/{max_iter} "
            f"loglik={new_loglik} d={np.asarray(d).tolist()} dp={np.asarray(dp).tolist()}"
        )
...
def _fit_copy_obs(em, obs: np.ndarray,
    ...
    with mp.Pool(
        ...
        # main loop
        for res in tqdm(pool.imap_unordered(fit_quadruplet_wrapper, args, chunksize=12),
                        total=total_tasks, desc="Running inference", smoothing=0.1):
            results.append(res)
    return results
...
def _fit_copy_obs_async(
    ...
    with mp.Pool(
        ...
        for a in args_gen:
            ar = pool.apply_async(fit_quadruplet_wrapper, (a,), callback=collect_result)
            async_results.append(ar)
        pool.close()
        pool.join()

    # Force retrieval to surface worker exceptions deterministically.
